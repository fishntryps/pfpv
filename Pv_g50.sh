...
##############################################################################################################
### the script tabulates frequency of P. vivax >0.50 IBD sharing within and between epidemiological zones ####
##############################################################################################################

cd /seq/plasmodium/pschwabl/Pv_IBD_g50_v4/

cut -f1-10 ../Pv_IBD_g09658/Pv_test_for_R_only_p025_mono.txt > myIBD.txt

##### requisites
cp ../Pv_IBD_g50_v3/CLEAN* .
cp ../Pv_IBD_g50_v3/degr* .
sed -re 's/^(Venez.*)Cristinas_Border(.*)/\1Venezuela\2/g' ../Pv_IBD_g50_v3/COORDS_grouped_v3.tab > COORDS_grouped_v4.tab
...
cat Pv_groups_v4.txt | while read i; do awk '$7=="'$i'"' COORDS_grouped_v4.tab | awk '{sum1+=$2; sum2+=$3} END {print  "'$i'\t"sum1/NR"\t"sum2/NR}' ; done > groups_v4_centroids.txt

cat Pv_groups_v4.txt | while read i; do cat Pv_groups_v4.txt | while read j; do awk '($12=="'$i'" && $14=="'$j'") || ($14=="'$i'" && $12=="'$j'")' myIBD_grouped_v4.txt > "$i"_"$j"_grocomp_v4.txt; awk '$3>"'$threshold'"'  "$i"_"$j"_grocomp_v4.txt | wc -l | awk '{print "'$i'\t'$j'\t"$0}' | paste - <( cat "$i"_"$j"_grocomp_v4.txt | wc -l); done; done | awk ' $4!=0 {print $0"\t"$3/$4}' | while read line; do echo "$line" | cut -f1,2 | perl -pe 's/\t/\n/g' | sort -V | perl -pe 's/\n/\t/g' | paste - <(echo "$line" | cut -f3-); done |  sort | uniq | sort -nk5,5 > perc_g"$threshasperc"_amoung_groups_v4.txt
...
paste <(echo "$i") both_"$L1"_"$L2"_numcc.txt; done > Pv_perc_g"$threshasperc"_amoung_groups_v4_with_distances_and_indcount_and_cccount.txt



cat Pv_groups_v4.txt | while read i; do awk '{print $1"\t"$12"xxx"$2"\t"$14}' myIBD_grouped_v4.txt | perl -pe 's/xxx/\n/g' | sort | uniq | egrep -c "[[:space:]]$i$" | awk '{print "'$i'\t"$0}'; done > Pv_sampsizes_grouped_v4_myIBD.txt

cut -f1 Pv_sampsizes_grouped_v4_myIBD.txt | while read i; do awk '$3 > "'$threshold'" && $12=="'$i'" && $14=="'$i'" {print $1"xxx"$2}' myIBD_grouped_v4.txt | perl -pe 's/xxx/\n/g' | sort | uniq | wc -l; done | paste Pv_sampsizes_grouped_v4_myIBD.txt - | sed '1iloc\tnumsamps\tnumlocallyclonal' > Pv_sampsizes_grouped_v4_myIBD_with_locallyclonalnums.txt